...
- [fan_control](./fan_control.py): A script that manages the CPU and GPU fan speeds through the `asus-nb-wmi` kernel module, that exposes a custom fan curve feature based on temperatures and PWM.
- [gpu-mode](./gpu-mode.py): A script to control the discrete GPU (nvidia) power state. On Muxless models (like the GA401IV), it allows seamlessly turn on or off the dGPU without rebooting or restarting the session. This is useful to save power when the dGPU is not needed.
